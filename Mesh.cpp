...
Mesh::Mesh(Vertex vertices[], size_t vertexCount, unsigned int indices[], size_t indexCount, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	CreateMeshBuffers(vertices, vertexCount, indices, indexCount, device);
}
...
Mesh::Mesh(const std::wstring& objFile, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	...
	CreateMeshBuffers(verts.data(), vertCounter, indices.data(), indexCounter, device);
}
...
void Mesh::CreateMeshBuffers(Vertex vertices[], size_t vertexCount, unsigned int indices[], size_t indexCount, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		vbd.ByteWidth = sizeof(Vertex) * (UINT)vertexCount;
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		ibd.ByteWidth = sizeof(unsigned int) * (UINT)indexCount;
